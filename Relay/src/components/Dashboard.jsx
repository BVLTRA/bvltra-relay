...
const Dashboard = () => {
  ...
  const fetchFaults = async () => {
    ...
    } catch (error) { console.error('Failed to fetch faults'); }
  };
  ...
  const handleSubmitIssue = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Reset form
        setEquipment(''); setDescription(''); setShift(''); setArea(''); setActionToBeTaken('');
        setPriority('Medium'); setTagType('Mechanical'); clearImage();
      }
    } catch (error) { console.error('Failed to log issue'); } 
          ...
          <form className="issue-form" onSubmit={handleSubmitIssue}>
            ...
            <div className="form-row">
              <div className="input-group">
                ...
                <input type="text" placeholder="e.g. Packaging, Brew House" value={area} onChange={(e) => setArea(e.target.value)} required />
              </div>
              <div className="input-group">
                <label>Shift</label>
                <input type="text" placeholder="e.g. Morning, Night, Shift A" value={shift} onChange={(e) => setShift(e.target.value)} required />
              </div>
            </div>
            ...
            <div className="form-row">
              <div className="input-group">
                ...
                <input type="text" placeholder="e.g. Filler Line 3, Conveyor B" value={equipment} onChange={(e) => setEquipment(e.target.value)} required />
              </div>
              <div className="input-group">
                <label>Date Raised</label>
                <input type="date" value={dateRaised} onChange={(e) => setDateRaised(e.target.value)} required />
              </div>
            </div>
            ...
            <div className="form-row">
              <div className="input-group">
                ...
                <div className="radio-group">
                  <label><input type="radio" value="Mechanical" checked={tagType === 'Mechanical'} onChange={(e) => setTagType(e.target.value)} /> Mechanical</label>
                  <label><input type="radio" value="Electrical" checked={tagType === 'Electrical'} onChange={(e) => setTagType(e.target.value)} /> Electrical</label>
                  <label><input type="radio" value="Safety" checked={tagType === 'Safety'} onChange={(e) => setTagType(e.target.value)} /> Safety</label>
                </div>
              </div>
              <div className="input-group">
            ...
            <div className="input-group">
              ...
              <textarea placeholder="Describe the exact issue..." value={description} onChange={(e) => setDescription(e.target.value)} rows="3" required></textarea>
            </div>

            <div className="input-group">
              <label>Description of Action to be Taken</label>
              <textarea placeholder="What steps need to be taken to resolve this? (Optional)" value={actionToBeTaken} onChange={(e) => setActionToBeTaken(e.target.value)} rows="2"></textarea>
            </div>
              ...
              {!imagePreview ? (
                ...
                >
                  <div className="drop-zone-content">
                    ...
                    <p style={{ fontSize: '0.8rem', color: '#666' }}>Mobile users: Click to activate camera or gallery.</p>
                  </div>
                  {/* Hidden input */}
                  <input 
                    type="file" 
                ...
                <div className="image-preview-container">
                  ...
                  <button type="button" className="remove-image-btn" onClick={clearImage}>Remove Image</button>
                </div>
              )}
            </div>
              ...
              faults.map(fault => (
                <div key={fault._id} className="fault-card">
                  <div className="fault-card-header">
                    <span className={`status-badge ${fault.status.toLowerCase().replace(' ', '-')}`}>{fault.status}</span>
                    <span className="timestamp">{new Date(fault.dateRaised || fault.createdAt).toLocaleDateString()} — {fault.shift} Shift</span>
                  </div>
                  <div className="fault-card-body">
                    ...
                    {fault.imageUrl && (
                      <div className="feed-image-container">
                        <img src={`http://localhost:5000${fault.imageUrl}`} alt="Fault Evidence" className="feed-image" />
                      </div>
                    )}
